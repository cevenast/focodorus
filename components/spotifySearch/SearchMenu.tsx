...
const SearchMenu = ({ token, searchMenuVisible, setSearchMenuVisible, setPlayer }:SearchMenu) => {
  ...
  return(
    <div className={`${searchMenuVisible ? '' : 'hidden'} absolute top-0 w-[380px] h-[550px] pt-8 rounded-xl  bg-gradient-to-b from-neutral-700 from-0% to-neutral-900 to-15% text-white`}>
      <button className="absolute h-10 w-10 right-2 top-4 rounded-full" onClick={() => setSearchMenuVisible(false)} >X</button>
